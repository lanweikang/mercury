define(
		"common/search",
		[ "bui/common", "bui/grid", "bui/form", "bui/data", "bui/overlay" ],
		function(e) {
			...
			g.augment(c, {
				...
				_initDomEvent : function() {
					var h = this, k = h.get("btnId"), i = h.get("store"), j = h
							.get("form");
				...
				_initForm : function() {
					...
					if (!j) {
						var i = g.merge(h.get("formCfg"), {
							srcNode : "#" + h.get("formId")
						});
						j = new d.HForm(i);
						j.render();
						h.set("form", j)
				...
				load : function(k) {
					var h = this, j = h.get("form"), i = h.get("store"), l = j
							.serializeToObject();
					if (k) {
						l.start = 0;
						l.pageIndex = 0
